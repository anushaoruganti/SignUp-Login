let prodData=[
  ...
  home.addEventListener("click",()=>{
    displayingCards(prodData)
  })

  function displayingCards(data){
    ...
    data.forEach(x=>{
        ...
        <h1>${x.title}</h1>
        <p>${x.price}</p>
        <button id="favorite" class="favorite" >❤️</button>
        <button class="addtocart">Add To Cart</button>
        `
    cardsContainer.appendChild(card)
        let favbtn=card.querySelector(".favorite")
...
function showFavsInTable(items,title){
    cardsContainer.innerHTML=""
    tableContainer.innerHTML=`<h1>${title}</h1>`
    if (items.length ===0){
    ...
    // deletebtns.forEach(singleButton => {
